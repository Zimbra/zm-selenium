...
Selenium.prototype.doClickZButton = function(locator, verifyParam) {
	/**
	 ...
	 */
//	if (locator.indexOf("=") > 0) {//use default elementFinder
//		var element = this.browserbot.findElement(locator);
//		this.browserbot.clickElement(element);
//	} else {
		var element = this.browserbot.findZButton(locator);
...
Selenium.prototype.doWaitZButton = function(locator, verifyParam) {
	/**
	 * Waits for Zimbra button
	 *
	 * @param locator button label
	 * @param verifyParam verify_text verify_btn verify_tab verify_dlg verify_view verify_menu verify_tbl verify_fldr
	 */
	if (verifyParam != "")
		locator = locator + ";" + verifyParam;

	return this._addWaitDecorator("verify_btn:=" + locator);

};
Selenium.prototype.doWaitZFolder = function(locator, verifyParam) {
	/**
	 * Waits for Zimbra Folder
	 *
	 * @param locator button label
	 * @param verifyParam verify_text verify_btn verify_tab verify_dlg verify_view verify_menu verify_tbl verify_fldr
	 */
	...
	return this._addWaitDecorator("verify_fldr:=" + locator);
};
...
Selenium.prototype.doOpenNewWindowByIconBtn = function(locator, verifyParam) {
	/**
...
Selenium.prototype.doClickMenuItem = function(menuItemName, verifyParam) {
	/**
	 * Clicks Zimbra menuItem
	 *
	 * @param menuItemName menu item's name
	 * @param verifyParam verify_text verify_btn verify_tab verify_dlg verify_view verify_menu verify_tbl verify_fldr
	 */
	var element = this.browserbot.findZMenuItem(menuItemName);
	this.browserbot.clickZElement(element);
	if (verifyParam.indexOf(":=") >= 0)
		return this._addWaitDecorator(verifyParam);
...
Selenium.prototype.doClickZFolder = function(locator, verifyParam) {
	/**
	 * Waits for Zimbra Folder
	 *
	 * @param locator Folder label
	 * @param verifyParam verify_text verify_btn verify_tab verify_dlg verify_view verify_menu verify_tbl verify_fldr
	 */
	this.browserbot.appendChildToView();//appends child to convView

	var element = this.browserbot.findZFolder(locator);
	this.browserbot.clickZElement(element);

	if (verifyParam.indexOf("verify_perf:=") >= 0)
		this.doLogPerfMessage(verifyParam.replace("verify_perf:=", ""));

	return Selenium.decorateFunctionWithTimeout(fnBind(this._verifyViewHasNoChild, this), 30000);
