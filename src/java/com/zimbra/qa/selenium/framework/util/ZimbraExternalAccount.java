...
public class ZimbraExternalAccount extends ZimbraAccount {
	...
	/**
	 * Based on the external invitation message, set both the login and registration
	 * URLs for this account
	 * 
	...
	/**
	 * Based on the external invitation message, extract the login URL example,
	...
	public void setRegistrationURL(Element GetMsgResponse) throws HarnessException {
		String content = this.soapClient.selectValue(GetMsgResponse, "//mail:mp[@ct='text/html']//mail:content", null,
				1);
		try {
	...
	/**
	 * Based on the external invitation message, extract the login URL example,
	 * https://server/?virtualacctdomain=server
	 * 
	 * @param GetMsgResponse
	 * @throws HarnessException
	 */
	...
	public static final Pattern patternLink = Pattern
			.compile("\\s*(?i)href\\s*=\\s*(\"([^\"]*\")|'[^']*'|([^'\">\\s]+))");

	/**
	 * Parse the invitation message to grab the registration URL for the external
	 * user
	 * 
	 * @param content
	 * @return
	...
	/**
	 * Parse the invitation message to grab the login URL for the external user
	 * 
	 * @param content
	 * @return
	 * @throws HarnessException
