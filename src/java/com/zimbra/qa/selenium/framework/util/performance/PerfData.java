...
public class PerfData {
	...
	/**
	 * The original value of Key_loaded and Key_Launched (so the harness can
	 * determine when a new value is set)
	 */
	...
	public String prettyPrint() {
		...
		if (Integer.parseInt(rDelta) > PerfMetrics.MaximumDeltaMSec) {
			return (String.format("%s, %s, %s, %s, %s, %s, %s", Key, "" + StartStamp, LaunchStamp, FinishStamp, rDelta,
					"0", "Error: Delta too long (" + PerfMetrics.MaximumDeltaMSec + " max)"));
		}
