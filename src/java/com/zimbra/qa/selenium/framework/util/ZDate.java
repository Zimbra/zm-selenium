...
public class ZDate {
	 ...
	 */
	public ZDate(int year, int month, int monthday, int hour, int minutes, int seconds, String timezone)
			throws HarnessException {
	...
	/**
	 * Create a ZDate object by parsing a Zimbra SOAP element, such as
	 * <s d="20140101T070000" u="1388577600000" tz="America/New_York"/>
	 * 
	 * @param e
	 * @throws HarnessException
	 */
	...
	public String getCurrentDay() throws HarnessException {
		...
		} else if (day == 5 || day == 6) { // see CalendarWorkWeekTest, If UTC is Friday, tests may fail if the TZ
											// offset puts the date into Saturday.
			currentDay = "Thursday";
	...
	public ZDate addSeconds(int amount) {
		...
		ZDate other = new ZDate(this.calendar.get(Calendar.YEAR), this.calendar.get(Calendar.MONTH) + 1,
				this.calendar.get(Calendar.DAY_OF_MONTH), this.calendar.get(Calendar.HOUR_OF_DAY),
				this.calendar.get(Calendar.MINUTE), this.calendar.get(Calendar.SECOND), this.calendar.getTimeZone());
