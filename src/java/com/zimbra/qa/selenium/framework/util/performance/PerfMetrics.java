...
public class PerfMetrics {
	...
	// Setting a limit helps with creating the charts - so the axes are not too
	// large.
	//
	...
	public static void waitTimestamp(PerfToken token) throws HarnessException {
		...
		if ((Long.parseLong(data.FinishStamp) - data.StartStamp) > MaximumDeltaMSec) {
			throw new HarnessException(
					"Delta duration is too long: " + (Long.parseLong(data.FinishStamp) - data.StartStamp));
		}
	...
	/**
	 * Return a map of URL query parameters, required to enable perf metrics from
	 * the Zimbra ajax app. See config.properties performance.metrics.query value
	 * (per http://bugzilla.zimbra.com/show_bug.cgi?id=61972#c11 : perfMetric=1)
	 * 
	 * @return
	 */
	public Map<String, String> getQueryMap() {
		...
		String appPoperty = ConfigProperties
				.getStringProperty("performance.metrics.query." + ConfigProperties.getAppType(), null);
		if (appPoperty != null) {
	...
	private String getValue(String id) {
		String value = "";
		try {
			value = (String) ((JavascriptExecutor) ClientSessionFactory.session().webDriver()).executeScript(
					"this.browserbot.getCurrentWindow().document.getElementById('" + id + "').innerHTML");
		} catch (Exception e) {
			logger.debug(e);
		}
