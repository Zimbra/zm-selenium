...
/**
 * This class contains general utilities methods that can be used across the
 * framework
 * 
...
public class GeneralUtility {
	...
	/**
	 * Dynamically waits either for the correct status or timeout, either one, which
	 * is faster. Example: Object[] input = null; DesktopInstallUtil test = new
	 * DesktopInstallUtil();
	 * 
	 * Below is the static method
	 * GeneralUtility.waitFor("projects.desktop.core.DesktopInstallUtil", null,
	 * true, "counter", input, WAIT_FOR_OPERAND.EQ, new Integer(10), 10000, 1000);
	 * 
	 * Below is the non-static method
	 * GeneralUtility.waitFor("projects.desktop.core.DesktopInstallUtil", test,
	 * false, "counter1", input, WAIT_FOR_OPERAND.EQ, new Integer(10), 10000, 1000);
	 *
	 * Where definition of those 2 methods are: public static int counter() {
	 * _count++; return _count; } public int counter1() { _count += 2; return
	 * _count; }
	 * 
	 * @param apiClassPath
	 *            API Class path, this is ONLY needed if the API being called is
	 *            static
	 * @param nonStaticObject
	 *            Object, who is the owner of the API, this is ONLY needed if the
	 *            API being called is non-static
	 * @param isStaticApi
	 *            Is it a static API
	 * @param apiName
	 ...
	 * @param operand
	 *            EQ (equal) or NEQ (not equal)
	 * @param comparingObject
	 *            Status of the object to be waited for
	 * @param timeout
	 *            Timeout to be waited for
	 * @param delayBetweenCheck
	 *            Delay between check until the timeout is hit
	 * @return Object from the last API call before exiting this method
	 * @throws HarnessException
	 */
	public static Object waitFor(String apiClassPath, Object nonStaticObject, boolean isStaticApi, String apiName,
			Object[] parameters, WAIT_FOR_OPERAND operand, Object comparingObject, long timeout, long delayBetweenCheck)
			throws HarnessException {
	...
	/**
	 * This method is only compliant for waitFor method with goal to compare the
	 * objects based on the passed operand
	 * 
	 * @param mainObject
	 *            First object
	 * @param compObject
	 *            Second object
	 * @param operand
	 *            EQ(Equals) or NEQ(Not Equals)
	 * @return true if both objects are matched according to the operand, otherwise
	 *         false.
	 * @throws HarnessException
	 */
	 ...
	 * 
	 * @param taskName
	 *            Task's name to be queried
	 * @return true, if the task is running, otherwise, false
	 * @throws IOException
	 * @throws InterruptedException
	 * @throws HarnessException
	 */
	public static boolean findWindowsRunningTask(String taskName)
	 ...
	 * 
	 * @param owner
	 *            Page Object, which is the owner of the locator
	 * @param locator
	 *            Locator of the element
	 * @throws HarnessException
	 * @return true if element is present, or false if element is not present when
	 *         timeout is hit
	 */
	...
	public static boolean waitForElementPresent(Object owner, String locator, long timeout) throws HarnessException {
		...
		return (Boolean) GeneralUtility.waitFor(null, owner, false, "sIsElementPresent", params, WAIT_FOR_OPERAND.EQ,
				true, timeout, 1000);
	}
	...
	public static void untarBaseUpgradeFile(File tarFile, File dest) throws HarnessException, InterruptedException {
		...
		try {
			...
			while (tarEntry != null) {
				File destPath = new File(dest.toString().trim() + File.separatorChar + tarEntry.getName());
			...
			throw new HarnessException(
					"Getting IO Exception while untarring the file from: " + tarFile + " to: " + dest, ie);
		} finally {
			close(tin);
		}
	}
