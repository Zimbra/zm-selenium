...
public class CodeCoverage {
			...
			try {
				...
				while (iterator.hasNext()) {
					...
					if (!cumulativeCoverage.containsKey(key)) {
						...
						cumulativeCoverage.getJSONObject(key).put("coverage", updateCoverage(
								cumulativeCoverage.getJSONObject(key).getJSONArray("coverage"), nCoverage));
	...
	private void traceCoverage(JSONObject oldJSON, JSONObject newJSON) {
		...
		logger.info("CodeCoverage: URL=" + webDriver.getCurrentUrl());
			...
			if (oldJSON == null) {
				...
				while (iterator.hasNext()) {
					...
					if (oldJSON.containsKey(key)) {
						...
						// New file was covered, add 1 to the file count
						countFiles++;
	...
	private JSONArray getSourceFileContentAsJSONArray(String jsFilename) {
		...
		try {
			...
			URL url = new URL("https://" + ConfigProperties.getStringProperty("server.host") + "/" + jsFilename);
	...
	protected String jsCoverageTool = "/usr/local/bin/jscoverage";
	...
	public static class CodeCoverageWebSourceFiles {
		...
		public static final List<String> filenames = Arrays.asList("jscoverage-highlight.css", "jscoverage-ie.css",
				"jscoverage-throbber.gif", "jscoverage.css", "jscoverage.html", "index.html", "jscoverage.js");
					...
					try {
						...
						if (this.getClass().getResource("/" + filename) != null) {
							...
							stream = this.getClass()
									.getResourceAsStream("/com/zimbra/qa/selenium/framework/util/coverage/" + filename);
						}
						if (stream == null)
