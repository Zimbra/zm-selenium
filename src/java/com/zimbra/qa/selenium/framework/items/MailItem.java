...
public class MailItem implements IItem {
	...
	/**
	 * The tag names (tn attribute) associated with this mail (see soap.txt for
	 * details)
	 */
	...
	public static MailItem importFromSOAP(Element GetMsgResponse) throws HarnessException {
		...
		try {
			...
			Element contentTextPlain = ZimbraAccount.SoapClient.selectNode(m,
					"//mail:mp[@ct='text/plain']//mail:content");
			Element contentBodyHtml = ZimbraAccount.SoapClient.selectNode(m,
					"//mail:mp[@ct='text/html']//mail:content");
	...
	public static MailItem importFromSOAP(ZimbraAccount account, String query) throws HarnessException {
		...
		try {
			...
			account.soapSend("<SearchRequest xmlns='urn:zimbraMail' types='message'>" + "<query>" + query + "</query>"
					+ "</SearchRequest>");
	...
	/**
	 * Generate a sample mime from this object This String can be used in an
	 * <AddMsgRequest/>
	 */
