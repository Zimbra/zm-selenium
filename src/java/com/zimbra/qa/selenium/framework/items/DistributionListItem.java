...
public class DistributionListItem extends ContactItem implements IItem {
	...
	public static DistributionListItem createDistributionListItem(ZimbraAccount owner) throws HarnessException {
		...
		// Set the owner
		ZimbraAdminAccount.GlobalAdmin()
				.soapSend("<DistributionListActionRequest xmlns='urn:zimbraAccount'>" + "<dl by='id'>" + id + "</dl>"
						+ "<action op='addOwners'>" + "<owner by='name' type='usr'>" + owner.EmailAddress + "</owner>"
						+ "</action>" + "</DistributionListActionRequest>");

		// Add two accounts
		ZimbraAdminAccount.GlobalAdmin()
				.soapSend("<AddDistributionListMemberRequest xmlns='urn:zimbraAdmin'>" + "<id>" + id + "</id>" + "<dlm>"
						+ ZimbraAccount.AccountA().EmailAddress + "</dlm>" + "<dlm>"
						+ ZimbraAccount.AccountB().EmailAddress + "</dlm>" + "</AddDistributionListMemberRequest>");
	...
	public static DistributionListItem importFromSOAP(Element GetDistributionListResponse) throws HarnessException {
		...
		try {
			...
			Element getDistributionListResponse = ZimbraAccount.SoapClient.selectNode(GetDistributionListResponse,
					"//admin:GetDistributionListResponse");
			if (getDistributionListResponse == null)
				throw new HarnessException("Element does not contain GetDistributionListResponse: "
						+ GetDistributionListResponse.prettyPrint());

			Element dl = ZimbraAccount.SoapClient.selectNode(getDistributionListResponse, "//admin:dl");
			if (dl == null)
				throw new HarnessException(
						"Element does not contain a dl element: " + getDistributionListResponse.prettyPrint());
	...
	public static DistributionListItem importFromSOAP(String by, String value) throws HarnessException {
		...
		Element response = ZimbraAdminAccount.GlobalAdmin()
				.soapSend("<GetDistributionListRequest xmlns='urn:zimbraAdmin'>" + "<dl by='" + by + "'>" + value
						+ "</dl>" + "</GetDistributionListRequest>");
