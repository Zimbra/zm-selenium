...
public class ContactGroupItem extends ContactItem implements IItem {
	...
	public static ContactGroupItem importFromSOAP(Element GetContactsResponse) throws HarnessException {
		...
		try {
			...
			Element getContactsResponse = ZimbraAccount.SoapClient.selectNode(GetContactsResponse,
					"//mail:GetContactsResponse");
			if (getContactsResponse == null)
				throw new HarnessException(
						"Element does not contain GetContactsResponse: " + GetContactsResponse.prettyPrint());

			Element cn = ZimbraAccount.SoapClient.selectNode(getContactsResponse, "//mail:cn");
			if (cn == null)
				throw new HarnessException(
						"Element does not contain a cn element: " + getContactsResponse.prettyPrint());
	...
	public static ContactGroupItem createContactGroupItem(ZimbraAccount account) throws HarnessException {
		...
		account.soapSend("<CreateContactRequest xmlns='urn:zimbraMail'>" + "<cn >" + "<a n='type'>group</a>"
				+ "<a n='nickname'>" + groupname + "</a>" + "<a n='fileAs'>8:" + groupname + "</a>" + sb.toString()
	...
	public static ContactGroupItem importFromSOAP(ZimbraAccount account, String query) throws HarnessException {
		...
		try {
			...
			account.soapSend("<SearchRequest xmlns='urn:zimbraMail' types='contact'>" + "<query>" + query + "</query>"
					+ "</SearchRequest>");
			...
			account.soapSend(
					"<GetContactsRequest xmlns='urn:zimbraMail' >" + "<cn id='" + id + "'/>" + "</GetContactsRequest>");
	...
	public static class MemberItem {
		...
		/**
		 * This method is used to compare two MemberItems. All classes should return a
		 * String such as "email@domain.com".
		 * 
		 * @return
		 */
		protected String getNormalized() {
		...
		public boolean equals(Object o) {
			 ...
			 * <m value="f7042cb6-9fed-477c-ade1-20da53822ce3:257" type="C"/> Eventually, it
			 * would be good to verify the Account ID part, but it is difficult to track
			 * that information at the moment.
			 * 
			 * For now, just do a String Contains using the ID.
			 */
			return (other.getNormalized().contains(getNormalized())
