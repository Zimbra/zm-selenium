...
public class ReplyMsgWithTwoThreeLineTextSignature extends AjaxCommonTest {
	...
	@Bugs(ids = "45490")
	@Test( description = "Reply Msg with text signature with multiple line and Verify signature through soap", groups = { "functional" })
	public void ReplyMsgWithTextSignature_01() throws HarnessException {
		...
		app.zGetActiveAccount().soapSend(
				...
				+ "<content type='text/plain'>" + sigBody
				+ "</content>" + "</signature>"
				+ "</CreateSignatureRequest>");
		
		String SignatureId = ZimbraAccount.AccountZWC().soapSelectValue("//acct:CreateSignatureResponse/acct:signature", "id");
		
		
		app.zGetActiveAccount().soapSend(
				"<CreateSignatureRequest xmlns='urn:zimbraAccount'>"
				+ "<signature name='" + sigName1 + "' >"
				+ "<content type='text/plain'>" + sigBody1
				+ "</content>" + "</signature>"
				+ "</CreateSignatureRequest>");
		
		String SignatureId1 = ZimbraAccount.AccountZWC().soapSelectValue("//acct:CreateSignatureResponse/acct:signature", "id");
		
		app.zGetActiveAccount().soapSend(
		"<GetIdentitiesRequest xmlns='urn:zimbraAccount'>"+
		"</GetIdentitiesRequest>");
		
		String IdentityId =  ZimbraAccount.AccountZWC().soapSelectValue("//acct:GetIdentitiesResponse/acct:identity", "id");
		
		app.zGetActiveAccount().soapSend(
				"<ModifyIdentityRequest xmlns='urn:zimbraAccount'>" +
						"<identity id='" + IdentityId + "'>" + 
							"<a name='zimbraPrefDefaultSignatureId'>"+ SignatureId +"</a>"+
							"<a name='zimbraPrefForwardReplySignatureId'>"+ SignatureId1 +"</a>"+
						"</identity>"+
