...
public class DialogEditFilter extends AbsDialog {
	...
	public enum ConditionType {
		From, To, Cc, ToOrCc, Subject, HeaderNamed, Size, Date, Body, Attachment, ReadReceipt, AddressIn, Calendar, Social, Message, Address,
	}

	public enum ConditionConstraint {
		MatchesExactly, DoesNotMatchExcactly, Contains, DoesNotContain, MatchesWildcard, DoesNotMatchWildCard,
	}

	public enum FilterAction {
		KeepInInbox, Discard, MoveIntoFolder, TagWith, MarkAs, RedirectToAddress,
	}
	...
	public void zAddFilterCriteria(ConditionType type, ConditionConstraint constraint, String value)
		...
		if (count < 1)
			throw new HarnessException("couldn't find any filter condition rows!");
		rowLocator = "css=table[id='ZmFilterRuleDialog_conditions']>tbody>tr:nth-child(" + count + ")";
		...
		if (constraint.equals(ConditionConstraint.Contains)) {
			...
			throw new HarnessException(
					"Condition constraint is not implemented yet! Please enter condition constraint as 'Contains' or 'MatchesWildcard' only.");
		}
	...
	public void zAddFilterAction(FilterAction action, String... name) throws HarnessException {
		...
		case KeepInInbox:
			option = "Keep in Inbox"; // This option is by default selected. No need to do any action.
		...
		case TagWith:
			if (!name.equals(null)) {
				...
				this.sMouseMoveAt("css=div[id^='zti__ZmPickTagDialog'] td[id$='_textCell']:contains(" + name[0] + ")",
						"0,0");
				this.sClickAt("css=div[id^='zti__ZmPickTagDialog'] td[id$='_textCell']:contains(" + name[0] + ")",
						"0,0");
		...
		case MarkAs:
			if (name[0].equalsIgnoreCase("Read") || name[0].equalsIgnoreCase("Flagged")) {
				...
				if (name[0].equalsIgnoreCase("Flagged")) {
					...
					this.sClick(
							"css=table[id='ZmFilterRuleDialog_actions'] div[id*='SELECT']:contains(" + name[0] + ")");
					SleepUtil.sleepMedium();
					this.sMouseMoveAt(Locators.dropdownOptionCss + ":contains(" + option + ") td td[id$='_title']",
							"0,0");
					this.sClickAt(Locators.dropdownOptionCss + ":contains(" + option + ") td td[id$='_title']", "0,0");
		...
		case RedirectToAddress:
			if (!name[0].equals(null)) {
				...
				throw new HarnessException(
						"Email Address can't be null! Please entera a valid email address for redirection.");
			}

		default:
			throw new HarnessException("Filter action is invalid");
		}
	}
}