...
public class EditCos extends AdminCommonTest {
	...
	/**
	 * Testcase : Edit cos -  Administrator should be able to disabled two factor authentication at the COS level
	 * Steps :
	 * 1. Create an cos using SOAP.
	 * 2. Edit the two factor authentication attributes using UI Right Click.
	 * 3. Edit created COS >> Go to advanced tab >> Remove a check mark from option 'Enable two-factor authentication' displayed under 'Two Factor Authentication' section
	 * @throws HarnessException
	 */
	...
	public void EditCos_06() throws HarnessException {
		...
		app.zPageEditCOS.zPreferenceCheckboxSet(Button.B_ENABLE_TWO_FACTOR_AUTH,false);

		// Submit the form
		form.zSubmit();

		// Verify the enable two-factor authentication is set to false
		ZimbraAdminAccount.AdminConsoleAdmin().soapSend(
				"<GetCosRequest xmlns='urn:zimbraAdmin'>" +
						"<cos by='name'>"+cosName+"</cos>"+
				"</GetCosRequest>");
		Element response1 = ZimbraAdminAccount.AdminConsoleAdmin().soapSelectNode("//admin:GetCosResponse/admin:cos/admin:a[@n='zimbraFeatureTwoFactorAuthAvailable']", 1);
		ZAssert.assertNotNull(response1, "Verify the COS is edited successfully");
		ZAssert.assertStringContains(response1.toString(),"FALSE", "Verify the Enable two-factor authentication is set to false");

	}
	
	/**
	 * Testcase : Edit cos -  Administrator should be able to disable the ability to generate app specific passwords at COS level
	 * Steps :
	 * 1. Create an cos using SOAP.
	 * 2. Edit the two factor authentication attributes using UI Right Click.
	 * 3. Edit created COS >> Go to advanced tab >> Remove a check mark from option "Enable application passcodes"
	 * @throws HarnessException
	 */
