...
import com.zimbra.qa.selenium.projects.universal.pages.DialogDeleteTag.DialogDeleteTagID;
...
public class TreeCalendar extends AbsTree {
	...
	protected AbsPage zTreeItem(Action action, Button option, FolderItem folder) throws HarnessException {
		...
		if ((action == Action.A_RIGHTCLICK) && (option == Button.B_DELETE)) {
			...
			page = new DialogEditFolder(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
			...
			page = new DialogShare(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
	...
	protected AbsPage zTreeItem(Action action, Button option, TagItem folder) throws HarnessException {
		...
		if (action == Action.A_LEFTCLICK) {
			...
			return (new DialogRenameTag(MyApplication, ((UniversalPages) MyApplication).zPageCalendar));
	...
	public AbsPage zPressPulldown(Button pulldown, Button option) throws HarnessException {
		...
		if (pulldown == Button.B_TREE_FOLDERS_OPTIONS) {
			...
			if (option == Button.B_TREE_NEWFOLDER) {
				...
				page = new DialogShareFind(MyApplication, ((UniversalPages) MyApplication).zPageMail);

			} else if (option == Button.B_TREE_NEW_EXTERNAL_CALENDAR) {

				optionLocator = "css=div[id='ADD_EXTERNAL_CALENDAR']";
				page = new DialogAddExternalCalendar(MyApplication, ((UniversalPages) MyApplication).zPageMail);
	...
	public AbsPage zPressButton(Button button) throws HarnessException {
		...
		if (button == Button.B_TREE_NEWFOLDER) {
			...
			page = new DialogShareFind(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
	...
	private List<SavedSearchFolderItem> zListGetSavedSearchFolders(String top) throws HarnessException {
		...
		for (int i = 1; i <= count; i++) {
			...
			item.setId(id.replace(
					"zti__" + ((UniversalPages) MyApplication).zGetActiveAccount().EmailAddress + ":main_Mail__", ""));
	...
	public boolean zIsActive() throws HarnessException {
		...
		if (!((UniversalPages) MyApplication).zPageCalendar.zIsActive()) {
			((UniversalPages) MyApplication).zPageCalendar.zNavigateTo();
		}
