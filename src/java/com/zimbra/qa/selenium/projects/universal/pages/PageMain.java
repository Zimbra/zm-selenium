...
import com.zimbra.qa.selenium.projects.universal.pages.mail.PageMail;
...
public class PageMain extends AbsTab {
	...
	public void zNavigateTo() throws HarnessException {
		...
		if (!((UniversalPages) MyApplication).zPageLogin.zIsActive()) {
			((UniversalPages) MyApplication).zPageLogin.zNavigateTo();
		}
		
		((UniversalPages) MyApplication).zPageLogin.zLogin(ZimbraAccount.AccountZCS());
	}
	...
	public void zLogout() throws HarnessException {
		...
		((UniversalPages) MyApplication).zPageLogin.zWaitForActive();
		((UniversalPages) MyApplication).zSetActiveAccount(null);
	}
	...
	public void zCloseWindow(SeparateWindow window, String windowTitle, UniversalPages app) throws HarnessException {
		zCoreCloseWindow(window, windowTitle, app);
	}

	public void zCloseWindow(SeparateWindowFormMailNew window, String windowTitle, UniversalPages app)
			throws HarnessException {
		zCoreCloseWindow(window, windowTitle, app);
	}

	public void zCloseWindow(SeparateWindowDisplayMail window, String windowTitle, UniversalPages app)
			throws HarnessException {
		zCoreCloseWindow(window, windowTitle, app);
	}

	public void zCloseWindow(SeparateWindowOpenAttachment window, String windowTitle, UniversalPages app)
			throws HarnessException {
		zCoreCloseWindow(window, windowTitle, app);
	}

	private void zCoreCloseWindow(AbsSeparateWindow window, String windowTitle, UniversalPages app)
			throws HarnessException {
		if (window != null) {
	...
	public void zCloseWindow(SeparateWindow window, UniversalPages app) throws HarnessException {
		zCoreCloseWindow(window, app);
	}

	public void zCloseWindow(SeparateWindowOpenAttachment window, UniversalPages app) throws HarnessException {
		zCoreCloseWindow(window, app);
	}

	private void zCoreCloseWindow(AbsSeparateWindow window, UniversalPages app) throws HarnessException {
		if (window != null) {
			window.zCloseWindow();
			window = null;
		}
	...
	public AbsTab zGetCurrentApp() throws HarnessException {
		...
		if (sIsVisible(inbox)) {
			return ((UniversalPages) MyApplication).zPageMail;
		} else if (sIsVisible(contacts)) {
			return ((UniversalPages) MyApplication).zPageContacts;
		} else if (sIsVisible(calendar)) {
			return ((UniversalPages) MyApplication).zPageCalendar;
		} else if (sIsVisible(tasks)) {
			return ((UniversalPages) MyApplication).zPageTasks;
		} else if (sIsVisible(briefcase)) {
			return ((UniversalPages) MyApplication).zPageBriefcase;
		} else if (sIsVisible(generalPreferencesOverviewPane)) {
			return ((UniversalPages) MyApplication).zPagePreferences;
		} else {
	...
	public void zCheckAppLoaded(String appIdentifier) throws HarnessException {
		...
		if (!appTab.zIsActive()) {
			...
			for (int i = 0; i <= 2; i++) {
				if (appTab.equals(((UniversalPages) MyApplication).zPageCalendar)) {
					SleepUtil.sleepMedium();
				} else {
					SleepUtil.sleepSmall();
				}
