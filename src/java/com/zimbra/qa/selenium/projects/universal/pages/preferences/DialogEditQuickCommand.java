	...
	throws HarnessException
	{
		...
		if ( target.equals(QuickCommandTarget.Browse) ) {
			...
			if ( operation.equals("Move into folder") ) {
				...
				page = new DialogMove(MyApplication, ((UniversalPages) MyApplication).zPagePreferences);
				
			} else if ( operation.equals("Tag with") ) {
				
				page = new DialogTag(MyApplication, ((UniversalPages) MyApplication).zPagePreferences);
