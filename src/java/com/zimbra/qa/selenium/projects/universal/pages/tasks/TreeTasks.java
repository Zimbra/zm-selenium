...
package com.zimbra.qa.selenium.projects.universal.pages.tasks;
...
public class TreeTasks extends AbsTree {
	...
	public AbsPage zPressButton(Button button) throws HarnessException {
		...
		if (button == Button.B_TREE_NEWTAG) {
			...
			page = new DialogTag(MyApplication,((UniversalPages) MyApplication).zPageTasks);
			
		} else if (button == Button.B_TREE_NEWTASKLIST) {

			locator = "css=div[id=ztih__main_Tasks__TASK] div[class^=ImgNewTaskList ZWidget]";
			page = new DialogCreateTaskFolder(MyApplication,((UniversalPages) MyApplication).zPageTasks);
	...
	public AbsPage zTreeItem(Action action, Button option, IItem tasklist) throws HarnessException {
		...
		if (option == Button.B_TREE_NEWTAG) {
			...
			page = new DialogWarning(DialogWarning.DialogWarningID.DeleteTagWarningMessage, MyApplication, ((UniversalPages) MyApplication).zPageTasks);

		} else if (option == Button.B_RENAME) {

			optionLocator = Locators.zRenameTagTreeMenuItem;

			page = new DialogRenameTag(MyApplication,((UniversalPages) MyApplication).zPageTasks);
	...
	public AbsPage zTreeItem(Action action, Button option, FolderItem folderItem) throws HarnessException {
		...
		if (option == Button.B_DELETE) {
			...
			page = new DialogRenameFolder(MyApplication, ((UniversalPages) MyApplication).zPageTasks);
			...
			page = new DialogShare(MyApplication,((UniversalPages) MyApplication).zPageTasks);
	...
	public boolean zIsActive() throws HarnessException {
		...
		if ( !((UniversalPages)MyApplication).zPageTasks.zIsActive() ) {
			((UniversalPages)MyApplication).zPageTasks.zNavigateTo();
		}
