...
import com.zimbra.qa.selenium.projects.ajax.pages.calendar.DialogConfirmDeleteRecurringAppointment;
...
import com.zimbra.qa.selenium.projects.universal.pages.calendar.DialogOpenRecurringItem.Confirmation;
...
public class PageCalendar extends AbsTab {
	...
	private AbsPage zListItemGeneral(String itemsLocator, Action action, String subject) throws HarnessException {
		...
		if ( action == Action.A_LEFTCLICK ) {
			...
			page = new DialogOpenRecurringItem(Confirmation.OPENRECURRINGITEM, MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
			if ( page.zIsActive() ) {
				return (page);
			}
	...
	private AbsPage zListItemMonthView(Action action, String subject) throws HarnessException {
		...
		if ( action == Action.A_LEFTCLICK ) {
			...
			DialogOpenRecurringItem openRecurring = new DialogOpenRecurringItem(Confirmation.OPENRECURRINGITEM, MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
	...
	private AbsPage zListItemListView(Action action, Button option, String subject) throws HarnessException {
		...
		if ( action == Action.A_RIGHTCLICK ) {
			...
			if (option == Button.O_OPEN_MENU) {
				...
				page = new DialogConfirmDeleteAppointment(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
	...
	private AbsPage zListItemGeneral(String itemsLocator, Action action, Button option, String subject) throws HarnessException {
		...
		if (action == Action.A_LEFTCLICK) {
			...
			if ( (option == Button.O_DELETE) || (option == Button.O_CANCEL_MENU) ) {
				...
				page = new DialogConfirmDeleteOrganizer(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
				if ( page.zIsActive() ) {
					return (page);
				}

				// If an attendee deletes an appointment, you get a "Confirm Delete" dialog with "Notify Organizer?"
				page = new DialogConfirmDeleteAttendee(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
				if ( page.zIsActive() ) {
					return (page);
				}
				...
				page = new DialogConfirmDeleteRecurringAppointment(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
				if ( page.zIsActive() ) {
					return (page);
				}
				...
				page = new DialogMove(MyApplication,((UniversalPages) MyApplication).zPageCalendar);
				...
				if (com.zimbra.qa.selenium.projects.universal.tests.calendar.meetings.attendee.singleday.actions.CreateACopy.organizerTest == false ||
					...
					page = new DialogInformational(DialogInformational.DialogWarningID.InformationalDialog, MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
				} else {
					page = null;
				}
			...
			if ( option == Button.O_NEEDS_ACTION_MENU ) {
				...
				page = new DialogTag(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
				...
				page = new DialogWarning(DialogWarning.DialogWarningID.ZmMsgDialog, MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
				...
				page = new DialogConfirmationDeclineAppointment(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
	...
	public AbsPage zListItem(Action action, Button option, Button subOption, String subject) throws HarnessException {
		...
		if (action == Action.A_RIGHTCLICK) {
			...
			page = new DialogConfirmationDeleteAppointment(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
			if ( page.zIsActive() ) {
				return (page);
			}
	...
	public AbsPage zToolbarPressButton(Button button) throws HarnessException {
		...
		if (button == Button.B_REFRESH) {
			...
			page = new DialogConfirmDeleteRecurringAppointment(MyApplication, ((AjaxPages) MyApplication).zPageCalendar);
			if ( page.zIsActive() ) {
				return (page);
			}
	...
	public AbsPage zKeyboardShortcut(Shortcut shortcut) throws HarnessException {
		...
		if ( shortcut == Shortcut.S_ASSISTANT ) {
			...
			page = new DialogAssistant(MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
			...
			page = new DialogCreateFolder(MyApplication, ((UniversalPages)MyApplication).zPageCalendar);
	...
	public void zNavigateTo() throws HarnessException {
		...
		((UniversalPages) MyApplication).zPageMain.zCheckAppLoaded(Locators.zCalendarFolder);
	}
	...
	public boolean zIsActive() throws HarnessException {
		...
		if (!((UniversalPages) MyApplication).zPageMain.zIsActive()) {
			((UniversalPages) MyApplication).zPageMain.zNavigateTo();
		}
	...
	public void zCreateTag(UniversalPages app, String tagName, int tagColor) throws HarnessException {
	...
	public boolean zVerifyMultidayAllDayAppointmentInMonthView(Calendar cal, int duration, String subject) throws HarnessException {
		...
		List<WebElement> elements = getElements("//div[@class='allday_item_filler']//ancestor::td[@class='calendar_month_cells_td']");
	...
	public boolean zVerifyAppointmentInMonthView(ZDate date, String subject) throws HarnessException {
		...
		List<WebElement> elements = getElements("//span[.='" + subject + "']//ancestor::td[@class='calendar_month_cells_td']");
	...
	public boolean zVerifyNonAllDayMultiDayAppointmentInMonthView(ZDate date, int noOfDays, String subject) throws HarnessException {
		...
		if(elements.size() != noOfDays ) {
			logger.error("number of dates which contain the appointment: " + elements.size() + " but it should be for " + noOfDays + "  1 days only!");
			return false;
		}

		//Verify the position of displayed appointments with respect to the position of date cell
		boolean b  = false;
	...
	public boolean zVerifyAppointmentPositionInMonthView(ZDate date, String subject) throws HarnessException {
		...
		element = getElement("//td[@class='calendar_month_day_label'][.='" + Integer.parseInt(date.toDD()) + "']//ancestor::td[@class='calendar_month_cells_td']");
		p = element.getLocation();

		//getting the height of the cell. Width of cell and appointment should be almost same
		...
		element = getElement("//td[@class='calendar_month_day_label'][.='" + Integer.parseInt(date.toDD()) + "']//ancestor::table[@class='calendar_month_day_table']"
				+ "/following-sibling::table//div//span[contains(text(),'"  + subject + "')]//ancestor::table[@role='presentation']");
		p = element.getLocation();
		...
		//X - co-ordinate matching--X positions of the appointment and day cell should be almost same
		...
		//Y - co-ordinate matching--Y position of the appointment should be within the height of the cell.
