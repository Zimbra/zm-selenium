...
import com.zimbra.qa.selenium.projects.universal.pages.ContextMenu;
import com.zimbra.qa.selenium.projects.universal.pages.DialogAddToBriefcase;
import com.zimbra.qa.selenium.projects.universal.pages.DialogAddToCalendar;
...
import com.zimbra.qa.selenium.projects.universal.pages.DialogTagPicker;
...
import com.zimbra.qa.selenium.projects.universal.pages.Tooltip;
...
public class PageMail extends AbsTab {
	...
	public void zNavigateTo() throws HarnessException {
		...
		((UniversalPages) MyApplication).zPageMain.zCheckAppLoaded(Locators.zInboxFolder);
	}
	...
	public AbsPage zToolbarPressButton(Button button) throws HarnessException {
		...
		if (button == Button.B_NEW) {
			...
			return (((UniversalPages) this.MyApplication).zPageMain.zToolbarPressButton(Button.B_REFRESH));
	...
	public AbsPage zListItem(Action action, Button option, String subject) throws HarnessException {
		...
		if (action == Action.A_RIGHTCLICK) {
			...
			if (option == Button.O_MARK_AS_READ) {
				...
				page = new DialogAddAttendees(this.MyApplication, ((UniversalPages) MyApplication).zPageCalendar);
	...
	public AbsPage zKeyboardShortcut(Shortcut shortcut) throws HarnessException {
		...
		if ((shortcut == Shortcut.S_NEWITEM) || (shortcut == Shortcut.S_NEWMESSAGE)
			...
			// "New Message" shortcuts result in a compose form opening
			// page = new FormMailNew(this.MyApplication);
			page = new DialogTag(MyApplication, ((UniversalPages) MyApplication).zPageMail);

		} else if ((shortcut == Shortcut.S_MAIL_TAG)) {

			// "New Message" shortcuts result in a compose form opening
			// page = new FormMailNew(this.MyApplication);
			page = new DialogTagPicker(MyApplication, ((UniversalPages) MyApplication).zPageMail);

		} else if ((shortcut == Shortcut.S_NEWFOLDER)) {

			// "New Message" shortcuts result in a compose form opening
			// page = new FormMailNew(this.MyApplication);
			page = new DialogCreateFolder(MyApplication, ((UniversalPages) MyApplication).zPageMail);
			...
			page = new DialogWarning(DialogWarning.DialogWarningID.PermanentlyDeleteTheItem, MyApplication,
					((UniversalPages) MyApplication).zPageMail);

		} else if (shortcut == Shortcut.S_ASSISTANT) {

			page = new DialogAssistant(MyApplication, ((UniversalPages) MyApplication).zPageMail);
			...
			page = new DialogWarning(DialogWarning.DialogWarningID.SaveCurrentMessageAsDraft, this.MyApplication,
					((UniversalPages) this.MyApplication).zPageMail);
	...
	public AbsPage zToolbarPressPulldown(Button pulldown, Button option, Object dynamic) throws HarnessException {
		...
		DialogWarning dialog = new DialogWarning(DialogWarning.DialogWarningID.ComposeOptionsChangeWarning,
				MyApplication, ((UniversalPages) MyApplication).zPageMail);
		if (dialog.zIsActive()) {
